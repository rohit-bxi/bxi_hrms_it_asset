...
class BxiDesktimeConfig(models.Model):
    ...
    def _process_employee_data(self, record_date, emp_data):
        ...
        # ── Create or update Timesheet (account.analytic.line) ─────────
        if productive_hours > 0:
            timesheet_vals = {
                'name': f'DeskTime: {record_date}',
                'employee_id': odoo_employee.id,
                'date': record_date,
                'unit_amount': productive_hours,
            }
            if self.default_project_id:
                timesheet_vals['project_id'] = self.default_project_id.id

            if log_record.timesheet_id:
                # Update existing timesheet
                log_record.timesheet_id.write(timesheet_vals)
            else:
                # Create new timesheet
                # Check if a DeskTime timesheet already exists for this employee/date
                existing_ts = self.env['account.analytic.line'].search([
                    ('employee_id', '=', odoo_employee.id),
                    ('date', '=', record_date),
                    ('name', 'like', 'DeskTime:'),
                ], limit=1)
                if existing_ts:
                    existing_ts.write(timesheet_vals)
                    log_record.timesheet_id = existing_ts.id
                else:
                    new_ts = self.env['account.analytic.line'].create(timesheet_vals)
                    log_record.timesheet_id = new_ts.id
