    ...
    def create_purchase_order(self, **params):
        try:
            ...
            for line in order_lines:
                ...
                if payload_product_id and payload_product_id not in [fixed_product_id, fixed_product_id_goods]:
                    return {
                        "status": "error",
                        "message": f"Only product_id {fixed_product_id} and {fixed_product_id_goods} allowed"
                    }

                if not qty:
                    return {"status": "error", "message": "product_qty is required"}

                if payload_product_id == fixed_product_id_goods:
                    product = request.env['product.product'].sudo().browse(fixed_product_id_goods)
                else:
