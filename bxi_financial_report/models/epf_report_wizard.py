...
class EpfReportWizard(models.TransientModel):
    ...
    def action_generate_excel(self):
        ...
        # Search payslips matching the selected pay period (date_from <= end_date and date_to >= start_date)
        payslip_domain = [
            ('state', '!=', 'cancel'),
        ...
        # Fallback if no payslips found for specific company: search across all active payslips in date range
        if not payslips:
            fallback_domain = [
                ('state', '!=', 'cancel'),
                ('date_from', '<=', end_dt_str),
